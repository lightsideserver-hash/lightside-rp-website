...
const roleIcons: Record<string, React.ReactNode> = {
  Tulajdonos: <Crown className="w-4 h-4" />,
  'Server Manager': <Shield className="w-4 h-4" />,
  Fejlesztő: <Wrench className="w-4 h-4" />,
}

const roleColors: Record<string, string> = {
  Tulajdonos: 'from-red-500 to-red-700',
  'Server Manager': 'from-orange-400 to-orange-600',
  Fejlesztő: 'from-yellow-300 to-yellow-500',
}
...
export const Team = () => {
  ...
  const gridColsLg = team.length === 3 ? 'lg:grid-cols-3' : 'lg:grid-cols-4'
      ...
      <div className="section-container relative z-10">
        ...
        <div className="text-center mb-16">
          <h2 ref={titleRef} className="text-4xl md:text-6xl font-gaming font-bold mb-4">
            <span className="gradient-text">Vezetőség</span>
          </h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">
            Ismerd meg a szerver mögött álló elhivatott csapatot
          </p>
        </div>
        ...
        >
          {team.map((member) => (
            <div
              key={member.id}
              className="team-card group w-full max-w-sm"
        ...
        {/* Csatlakozz a csapathoz CTA */}
        <div className="mt-16 text-center">
          <div className="inline-block">
            <p className="text-gray-400 mb-4">Szeretnél csatlakozni a csapatunkhoz?</p>
            <a
